...
/**
 * @brief Inverts a given dword using a specified matrix.
 *
 * This function takes a dword `x` and a matrix `M`, and attempts to invert `x`
 * using the matrix. If `x` is zero, the function returns zero immediately.
 * Otherwise, it populates the matrix `M` with rotated values of `x` and
 * corresponding powers of two. It then performs Gaussian reduction on the
 * matrix. If the reduction is successful, the function returns the inverted
 * value; otherwise, it returns zero.
 *
 * @param x The dword to be inverted.
 * @param M The matrix used for the inversion process.
 * @return The inverted dword if successful, otherwise zero.
 */
...
/**
 * @brief Permutes the input matrix `M` based on the permutation array `p` and stores the result in `M_IS`.
 *
 * This function takes a matrix `M` and permutes its rows based on the permutation array `p`. The result is stored in the matrix `M_IS`.
 * The permutation is performed using bitwise operations to efficiently handle the matrix elements.
 *
 * @param M The input matrix represented as a `dword`.
 * @param p An array of integers representing the permutation of the rows.
 * @param M_IS Pointer to the matrix structure where the permuted matrix will be stored.
 */
...
#pragma omp parallel
  {
    ...
    matrix Ma = {.m = MATRIX_SIZE, .n = 2 * MATRIX_SIZE, .bn = 2, .data = Mptr};

    // set the matrix data form one domain array
    for (int i = 0; i < MATRIX_SIZE; i++)
      Ma.data[i] = Mdata + 2 * i;
...
/**
 * @brief Performs Gaussian elimination on a matrix to reduce it to its row echelon form.
 *
 * This function takes a matrix and performs Gaussian elimination to transform it into its row echelon form.
 * It uses bitwise operations to efficiently handle the matrix elements.
 *
 * @param M Pointer to the matrix structure to be reduced.
 * @param cols Number of columns in the matrix.
 * @param last Number of rows at the bottom of the matrix that should not be modified.
 * @return int Returns 1 if the matrix has been fully reduced to row echelon form, 0 otherwise.
 */
int gauss_reduce(matrix *M, unsigned int cols, int last)
{
  /* column index */
